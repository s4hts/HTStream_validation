...
base_dir = '/share/biocore/keithgmitchell/master_wrapper_msa'
base_dir_summary = '/share/biocore/keithgmitchell/benchmarking'
...
def msa(true_sequence, raw_sequence, ec_sequence, description):
    ...
    with open('%s/%s.fasta' %(base_dir, id_temp), 'w') as fasta:
        fasta.write(">TRUE\n")
    ...
    # run the msa
    # TODO fix muscle extension
    subprocess.call(["/share/biocore/keith/benchmarking/tools/./muscle3.8.31_i86linux64",
                     "-in", "%s/%s.fasta" %(base_dir, id_temp),
                     "-out", "%s/%s.fasta" %(base_dir, id_msa)])

    # remove the temporary file from prior to the msa
    os.remove("%s/%s.fasta" % (base_dir,id_temp))

    # parse the msa file to get each sequences we care about
    aligned = {"RAW": '',"TRUE": '', "EC": ''}
    fasta_msa = SeqIO.parse("%s/%s.fasta" % (base_dir, id_msa), 'fasta')
    for rec in fasta_msa:
        aligned[rec.description] = rec.seq

    # remove the temporary file from after the msa
    os.remove("%s/%s.fasta" % (base_dir,id_msa))
...
def resolve_trim(true, raw, ec):
    ...
    # TODO should this be based on the true read instead? We care about the ec trimming here as it should match the true read hopefully.. so then if it is not - then
    # we move to reads where there is a base and penalize there
    # start from the left
...
def handle_sequences(true_rec, raw_rec, ec_rec, cleaned_filename, description):
    alignment = msa(true_rec, raw_rec, ec_rec)
    base_counts = analyze_bases(alignment[0], alignment[1], alignment[2])

    if base_counts is None:
        message = "FAILURE: Base count == 'None'(improper MSA) %s" % description
        # my_log(base_dir_join, cleaned_filename, message)
        ...
        store_base_data(base_dir_summary, cleaned_filename, description, length, base_stats)
        store_read_data(base_dir_summary, cleaned_filename, description, read_class)
        baseline(base_dir_summary, cleaned_filename, description, length, base_stats)
        data_compression(base_dir_summary, cleaned_filename, description, length, position_calls)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Produces EC Evaluation for either paired-end or single-end sequences. \
                                        Will error if paired end is not supplied for true, raw, and ec.')
    ...
    parser.add_argument('-ec_2', help='This is the ec file to be evaluated.', required=False)
    parser.add_argument('-pe', help='Default is TRUE. (supply TRUE or FALSE)', required=False)
    ...
    if true1_filename is None or raw1_filename is None or ec1_filename is None:
        sys.exit("Please supply True 1, Raw 1, and EC 1")

    if args['pe']:
        if true2_filename is None or raw2_filename is None or ec2_filename is None:
            sys.exit("PE data indicated but no PE data supplied")
    ...
    if checkEqual1([len(fastq_ec1_parser), len(fastq_ec2_parser), len(fastq_true1_parser),
                    len(fastq_true2_parser), len(fastq_ec2_parser), len(fastq_ec1_parser)]):
        for true1, raw1, ec1 in zip(fastq_true1_parser, fastq_raw1_parser, fastq_ec1_parser):
            if checkEqual1([true1.rec.description(), raw1.rec.description(), ec1.rec.description()]):
                handle_sequences(true1.seq, raw1.seq, ec1.seq, cleaned_filename, true1.rec.description())
        if args['pe']:
            for true2, raw2, ec2 in zip(fastq_true2_parser, fastq_raw2_parser, fastq_ec2_parser):
                if checkEqual1([true2.rec.description(), raw2.rec.description(), ec2.rec.description()]):
                    handle_sequences(true2.seq, raw2.seq, ec2.seq, cleaned_filename, true2.rec.description())
    ...
    # my_log(bcleaned_filename, message)
